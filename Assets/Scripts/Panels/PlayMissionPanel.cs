...
namespace Panels
{
    public class PlayMissionPanel : MissionPanel
    {
        ...
        protected override void OnActivate(IReadOnlyMission mission)
        {
            ...
            _playerSideText.text = string.Join(", ", mission.CurrentPlayerFractions.Select(f => f.ToRussianName()));
            _enemySideText.text = string.Join(", ", mission.CurrentEnemyFractions.Select(f => f.ToRussianName()));
        }
    }
}